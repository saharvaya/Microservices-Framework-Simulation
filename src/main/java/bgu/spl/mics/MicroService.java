...
public abstract class MicroService implements Runnable {
    ...
    //Fields
    private Map<Class, Callback> messageCallbacks; // Maps Message types to to appropriate message callback method.
    private volatile boolean terminated = false; // Determines whether this service has terminated
    private final String name; // Representing name for current service
    ...
    public MicroService(String name) {
        this.name = name;
        this.messageCallbacks = new Hashtable<>();
    }
    ...
    public final void run() {
        //Register to MessageBus and initialize current service including subscription to relevant messages
        MessageBusImpl.getInstance().register(this);
        initialize();
        //As long as this service is active, fetch new messages from the MessageBus if exists
        while (!terminated) {
            try {
            	processMessage(MessageBusImpl.getInstance().awaitMessage(this));
            } catch (InterruptedException e) {
                System.out.println("Service "+this.getName() +" Interrupted");
            }
        }
    }

    /**
     * A method used to retrieve the relevant callback method for the given message
     * and initiate the callback if this micro service is subscribed to it.
     * @param m the message to call the callback on.
     */
    private void processMessage(Message m)
    {
        if(m != null) {
            @SuppressWarnings("unchecked")
			Callback<Message> reaction = messageCallbacks.get(m.getClass());
            if(reaction != null)
                reaction.call(m);
        }
    }
